...
def main(gpu_ids, base_lr, eps, P, K, tau, beta, k1, sampling, lambda_hard, number_of_iterations, momentum_on_feature_extraction, 
	...
	for pipeline_iter in range(1, number_of_epoches+1):
		...
		# Ablation Study for clustering DBSCAN analysis
		#selected_images, pseudo_labels, ratio_of_reliable_data = DBSCANclustering(train_images_target, distances, eps=eps)

		
		
		selected_images_resnet50, pseudo_labels_resnet50, ratio_of_reliable_data_resnet50 = multiClustering(train_images_target, distances_resnet50)
		num_classes_resnet50 = len(np.unique(pseudo_labels_resnet50))
		calculatePurityAndCameras(selected_images_resnet50, pseudo_labels_resnet50)
		print("Number of classes: %d" % num_classes_resnet50)

		selected_images_osnet, pseudo_labels_osnet, ratio_of_reliable_data_osnet = multiClustering(train_images_target, distances_osnet)
		num_classes_osnet = len(np.unique(pseudo_labels_osnet))
		calculatePurityAndCameras(selected_images_osnet, pseudo_labels_osnet)
		print("Number of classes: %d" % num_classes_osnet)

		selected_images_densenet121, pseudo_labels_densenet121, ratio_of_reliable_data_densenet121 = multiClustering(train_images_target, distances_densenet121)
		num_classes_desnenet121 = len(np.unique(pseudo_labels_densenet121))
		calculatePurityAndCameras(selected_images_densenet121, pseudo_labels_densenet121)
		print("Number of classes: %d" % num_classes_desnenet121)
		

		del train_fvs_resnet50, train_fvs_osnet, train_fvs_densenet121
		del distances_resnet50, distances_osnet, distances_densenet121

		#lr_value = base_lr_values[pipeline_iter-1]*ratio_of_reliable_data
		
		# Learning Rate Warmup Ablation Study
		#if pipeline_iter <= 10:
		#	lr_value = base_lr*(pipeline_iter/10)
		#else:
		#	lr_value = base_lr

		lr_value_resnet50 = base_lr_values[pipeline_iter-1] #*ratio_of_reliable_data_resnet50
		print(colored("Learning Rate for ResNet50: %f" % lr_value_resnet50, "cyan"))
		lambda_lr_warmup(optimizer_resnet50, lr_value_resnet50)
		#lambda_lr_warmup(optimizer_resnet50, lr_value)

		lr_value_osnet = base_lr_values[pipeline_iter-1] #*ratio_of_reliable_data_osnet
		print(colored("Learning Rate for OSNet: %f" % lr_value_osnet, "cyan"))
		lambda_lr_warmup(optimizer_osnet, lr_value_osnet)
		#lambda_lr_warmup(optimizer_osnet, lr_value)

		lr_value_densenet121 = base_lr_values[pipeline_iter-1] #*ratio_of_reliable_data_densenet121
		print(colored("Learning Rate for DenseNet121: %f" % lr_value_densenet121, "cyan"))
		lambda_lr_warmup(optimizer_densenet121, lr_value_densenet121)
		#lambda_lr_warmup(optimizer_densenet121, lr_value)
	
		print(colored("Training ResNet50 ...", "green"))
		model_online_resnet50, model_momentum_resnet50, optimizer_resnet50 = train(selected_images_resnet50, pseudo_labels_resnet50, 
																					sampling, optimizer_resnet50, 
		...
		model_online_osnet, model_momentum_osnet, optimizer_osnet = train(selected_images_osnet, pseudo_labels_osnet, sampling, 
																			optimizer_osnet, 
																			P, K, perc, tau, beta, lambda_hard, 
																			number_of_iterations, 
																			model_online_osnet, 
		...
		model_online_densenet121, model_momentum_densenet121, optimizer_densenet121 = train(selected_images_densenet121, 
																					pseudo_labels_densenet121,
																					sampling, 
...
def DBSCANclustering(images, distances, eps=0.5):
	...
	selected_images = images[proposed_labels != -1]
	pseudo_labels = proposed_labels[proposed_labels != -1]

	ratio_of_reliable_data = pseudo_labels.shape[0]/proposed_labels.shape[0]
	print("Reliability: %.3f" % ratio_of_reliable_data)
...
if __name__ == '__main__':
	...
	parser.add_argument('--eps', type=float, default=0.5, help='Learning Rate')
	...
	main(gpu_ids, base_lr, eps, P, K, tau, beta, k1, sampling, lambda_hard, number_of_iterations, momentum_on_feature_extraction, 
															target, dir_to_save, dir_to_save_metrics, version, eval_freq)
